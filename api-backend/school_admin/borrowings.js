...
router.get('/', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {   //get a connection to the database from the pool
        ...
        WHERE (Borrowing.Returned = 1 OR Borrowing.BorrowDate >= DATE_SUB(CURRENT_DATE, INTERVAL 1 WEEK))
        AND ((CASE
            WHEN Student.IdUsers IS NOT NULL THEN Student.StudentName
            WHEN Teacher.IdUsers IS NOT NULL THEN Teacher.TeacherName
            ELSE 'Unknown'
        ...
        ORDER BY Users.IdUsers ASC;      
            ...
            connection.query(q, function (err, result) {
                if (err) {
                    ...
                    const filePath = "../frontend/views/school_admin/borrowings.ejs";
                    fs.readFile(filePath, 'utf8', function (err, html) {
                        if (err) {
                            ...
                            const renderedHtml = ejs.render(html, { user_id, school_id, borrowings: borrowings });
