...
router.get('/', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {   //get a connection to the database from the pool
        ...
        if (err) {
          ...
          JOIN Review r ON r.IdUsers = u.IdUsers
          WHERE u.Approved = 1 AND r.Approval = 0 AND (s.StudentName IS NOT NULL OR t.TeacherName IS NOT NULL);
          `;
            connection.query(q, function (err, result) {
                if (err) {
                    ...
                    connection.release();
                    const filePath = "../frontend/views/school_admin/approve_reviews.ejs";
                    fs.readFile(filePath, 'utf8', function (err, html) {
                        if (err) {
                            console.error('Error reading HTML file:', err);
                            res.status(500).send('Internal Server Error');
                        } else {
                            const renderedHtml = ejs.render(html, { user_id, school_id, successMessage, failureMessage, reviews: reviews });
